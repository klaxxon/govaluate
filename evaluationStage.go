...
func addStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	...
	if _, ok := left.([]*float64); ok {
		arr := make([]*float64, len(left.([]*float64)))
		for a := range arr {
			if left.([]*float64)[a] != nil && right.([]*float64)[a] != nil {
				f := *left.([]*float64)[a] + *left.([]*float64)[a]
				arr[a] = &f
			}
		}
		return arr, nil
...
func subtractStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	if _, ok := left.([]*float64); ok {
		...
		for a := range arr {
			if left.([]*float64)[a] != nil && right.([]*float64)[a] != nil {
				f := *left.([]*float64)[a] - *right.([]*float64)[a]
				arr[a] = &f
			}
		}
		return arr, nil
...
func multiplyStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	if isFloat64Array(left) || isFloat64Array(right) {
		// Two possible, both are arrays or one is array and other numeric
		if isFloat64Array(left) && isFloat64Array(right) {
			sum := 0.0
			for a := range left.([]*float64) {
				if left.([]*float64)[a] != nil && right.([]*float64)[a] != nil {
					sum += *left.([]*float64)[a] * *right.([]*float64)[a]
				}
			}
			return sum, nil
		} else {
...
func isFloat64Array(value interface{}) bool {
	switch value.(type) {
	case []*float64:
		return true
	}
